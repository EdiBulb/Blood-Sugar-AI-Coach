...
import apiClient from "../api/axios"; // ✅ axios 대신
...
export default function LogForm({ onSaved, setCoachMessage }) {
  const [valueMmol, setValueMmol] = useState("");
  const [dateISO, setDateISO] = useState(todayLocalISO());
  const [timeSlot, setTimeSlot] = useState(TIME_SLOTS[0]);
  const [mealState, setMealState] = useState(MEAL_STATES[0]);
  ...
  async function handleSubmit(e) {
    ...
    // ✅ 코치 메시지 받기
    const { data } = await apiClient.post("/coach", {
      value: mgdl,
      timeSlot,
      mealState,
    });
